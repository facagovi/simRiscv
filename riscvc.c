...
void execute() {
    ...
    case I_add:
        ...
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
        break;
    
    case I_addi:
        breg[rd] = breg[rs1] + imm32;
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
        break;
    ...
    // fetch ja soma 4 ent quando pula tem q fazer sub 4 e se nada nada
    case I_jal:
        breg[rd] = pc;
        pc = pc-4 + imm32;
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
    ...
    case I_jalr:
        breg[rd] = pc;
        pc = (breg[rs1] + imm32) & ~1; //certifica end par
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
        break;
    ...
    case I_lui:
        ...
        //SLT's retornam so 1 msm nn sei pq
    case I_slt:
    ...
    case I_slti:
        breg[rd] = ((int32_t)breg[rs1] < (int32_t)imm32) ? 1 : 0;
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
        break;
    ...
    case I_sltiu:
        breg[rd] = ((uint32_t)(breg[rs1]) < (uint32_t)(imm32)) ? 1 : 0;
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
        break;

    // Funfa sem SHAMT??? os de imediato
    // ve tbm se preenche certo se mudar pra sint
    case I_sll:
    ...
    case I_slli:
        breg[rd] = (uint32_t)breg[rs1] << imm32;
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
        break;

    case I_sra:
        breg[rd] = (int32_t)breg[rs1] >> breg[rs2];
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
        break;

    case I_srai:
        breg[rd] = (int32_t)breg[rs1] >> imm32;
        breg[rd] = read_reg(rd);// garante que x0 sempre seja 0
        break;
    ...
    case I_ecall: // reg17 = a7 reg10 = a0
        if (breg[17] == 1) { //print int
            printf("%d", breg[10]);
        } else if (breg[17] == 4) { //print string
            uint32_t str_addr = breg[10];
            while (mem[str_addr] != '\0') {
                putchar(mem[str_addr]);
                str_addr++;
            }
        } else if (breg[17] == 10) { // quit
            stop_prg = 1;
        }
        break;
 ...
 * Programa principal
 *
 * 0:  jal  x1, 16      // chama a função em PC = 16
 * 4:  addi x5, x0, 1   // executado após o retorno
 * 8:  jal  x0, 16      // salta para o final
 * 12: addi x0, x0, 0   // não deve ser executado
 *
 * Função
 *
 * 16: addi x6, x0, 2   // resultado da função
 * 20: jalr x0, 0(x1)   // retorna para PC = 4
 *
 * Final
 *
 * 24: addi x7, x0, 3
...
// Chamada da função
sw(0, 0, 0x010000EF);    // jal x1, 16

// Retorno da função
sw(0, 4, 0x00100293);    // addi x5, x0, 1

// Salta para o final, evitando executar a área intermediária
sw(0, 8, 0x0100006F);    // jal x0, 16

// Instrução que não deve ser executada
sw(0, 12, 0x00000013);   // nop


// Função chamada
sw(0, 16, 0x00200313);   // addi x6, x0, 2

// Retorno para o endereço armazenado em x1
sw(0, 20, 0x00008067);    // jalr x0, 0(x1)


// Final do programa
sw(0, 24, 0x00300393);   // addi x7, x0, 3
